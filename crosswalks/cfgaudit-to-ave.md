...
| | Version |
|---|---|
| cfgaudit | 1.12.0, read at `6cbddb5` |
| AVE record set | 80 records (AVE-2026-00001 .. 00080), read at `1d1d197` |
| Bawbel Scanner (validation, below) | 1.3.0 |

## Coverage

cfgaudit has **102 rules** in total. **61 of them map onto 27 AVE behavioral classes**, up from 53 onto 23 at v1.11.0. It is a many-to-one mapping: several cfgaudit rules land on the same AVE class, because cfgaudit slices threats by config surface where AVE slices by behavior. For example, cfgaudit has five distinct secret-detection rules (a secret in `settings.json` env, in an MCP `env`/`headers` block, an entropy fallback, a Continue inline `apiKey`, a crypto signing key), and all five map to the single AVE class `AVE-2026-00047` (hardcoded credentials in component).

The other 41 rules have no AVE class: they check config surfaces AVE's skill and MCP-server records do not enumerate (see "Config surfaces beyond AVE's model" below).

**Only two of the eight new mappings are new cfgaudit rules.** The other six are rules that existed all along and are mapped here for the first time, because the classes they belong to did not exist when the last crosswalk was generated: `AVE-2026-00071`, `AVE-2026-00072` and `AVE-2026-00073` were published on 2026-08-06 and `AVE-2026-00076` shortly after, while the v1.11.0 crosswalk was generated on 2026-08-05 and merged before it could pick them up. Three of those four came out of this crosswalk's own gap list ([#68](https://github.com/aveproject/ave/issues/68)), which is now down to **one** open surface.

**Three new rules are left unmapped on purpose.** `CFG100`, `CFG101` and `CFG102` shipped in v1.12.0 with no AVE id. In each case the nearest class is wrong on a stated detail. Reasons are under "Config surfaces beyond AVE's model".

**Both sides are pinned by commit**, so the counts can be re-derived. The cfgaudit tree read is two commits past the `v1.12.0` tag: [#516](https://github.com/cfgaudit/cfgaudit/pull/516) maps `CFG071` to `AVE-2026-00073`, [#518](https://github.com/cfgaudit/cfgaudit/pull/518) changes how `CFG101` groups findings. Neither adds or removes a rule, so 102 holds for the tag too. At the tag itself 60 rules are mapped, not 61.
...
| CFG094 | AVE-2026-00076 | natural-language steering of an approval classifier | Cursor `.cursor/permissions.json` `autoRun.allow_instructions` |
